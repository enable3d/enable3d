...
class MainScene extends Scene3D {
  ...
  async create() {
    ...
    const gltf = await this.load.gltf('/assets/hand_low_poly-test.glb')
    ...
    // this.pilar.mesh.geometry.copy(this.pilar.object3D.geometry)
    ...
    setTimeout(() => {
      this.physics.add.sphere({ x: 2, y: 15, radius: 0.5, mass: 1 })
      this.physics.add.sphere({ x: 4, y: 15, radius: 0.5, mass: 1 })
      const ball = this.physics.add.sphere({ x: 0, y: 15, radius: 1.5, mass: 1 })
      ball.body.setBounciness(0.5)
    }, 3000)

    console.log('bond', this.pilar.bone)
  }

  update(time: number) {
    this.pilar.bone.bones.forEach(b => {
      // console.log(b)
      b.rotation.x += 0.01
    })
    return
